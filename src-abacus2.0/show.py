...
i=0
for cell in cells:
	ax.add_patch(plt.Rectangle((cell.lx, cell.ly), cell.width, cell.height, facecolor = cell.color(), linewidth = 0.1, edgecolor = 'cyan', alpha = 0.4))
	X = [cell.oldlx, cell.lx]
	Y = [cell.oldly, cell.ly]
	ax.plot(X, Y, linewidth = 0.1, color = 'black', linestyle = '--')
	ax.text(cell.lx, cell.ly, s = str(i), fontsize = 1)
	i+=1
